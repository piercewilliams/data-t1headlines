...
def main() -> int:
    ...
    _SKIP_EXP = {"README.md", "experiment_log.md"}
    _exp_specs = sorted(
        f for f in Path("experiments").glob("*.md") if f.name not in _SKIP_EXP
    ) if Path("experiments").exists() else []
    ...
    # 4c. Regenerate style guide (shareable 1-pager of confirmed rules)
    _sg_result = subprocess.run(["python3", "generate_style_guide.py"])
    if _sg_result.returncode != 0:
        print("⚠ generate_style_guide.py failed — style guide may be stale")
    else:
        print("✓ Style guide written to docs/style-guide/index.html")
