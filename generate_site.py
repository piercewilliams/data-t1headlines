...
# ── Snowflake enrichment (optional — generated by snowflake_enrich.py) ────────
# Provides per-article O&O traffic + per-author pub/topic breakdowns.
# Falls back gracefully to empty dicts if the file doesn't exist yet.
_SF_ENRICH_PATH = Path("data/snowflake_enrichment.json")
if _SF_ENRICH_PATH.exists():
    _sf_raw        = json.loads(_SF_ENRICH_PATH.read_text(encoding="utf-8"))
    SF_ARTICLES    = _sf_raw.get("articles", {})   # url_norm → enrichment dict
    SF_AUTHORS     = _sf_raw.get("authors", {})    # author_name → aggregates dict
    SF_GENERATED   = _sf_raw.get("generated", "")
    print(f"Loaded snowflake_enrichment.json ({len(SF_ARTICLES)} articles, "
          f"{len(SF_AUTHORS)} authors, generated {SF_GENERATED[:10]})")
else:
    SF_ARTICLES  = {}
    SF_AUTHORS   = {}
    SF_GENERATED = ""
    print("snowflake_enrichment.json not found — Snowflake enrichment skipped.")
