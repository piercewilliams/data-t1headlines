...
- **Generator:** `generate_site.py` — run via `ingest.py`; `_build_nav()` / `_NAV_PAGES` single source of truth (5 nav pages now including Headline Grader); `EXCLUDE_MSN = False`; experiments page generated by `_collect_experiment_suggestions()` / `_generate_experiments_page()` / `_append_experiment_log()`
