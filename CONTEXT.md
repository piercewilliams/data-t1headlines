...
**Status:** Active — monthly Tarrow cadence + weekly ANP drops + daily grader cron
**Last session:** 2026-04-06 (Headline Grader shipped; brand cleanup; 30-day history; full hardening pass)
...
- **Headline Grader:** `docs/grader/index.html` — daily auto-grader for Sara Vallone's Tracker headlines; 14 criteria (rule-based + Groq LLM); 30-day rolling history at `docs/grader/history.json`; cron at 10am Chicago via `run_grader.sh`; credentials in `~/.grader_env` (never committed)
